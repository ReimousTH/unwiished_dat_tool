...
void DatFile::ReadFile()
{
    ...
    // FIX: Operator precedence issue - add parentheses
...
void DatFile::WriteFile()
{
    ...
    root.Write(this);
    auto marks = this->get_mark("#string_");
    for (auto& pair : marks)
    {
...
void DirectoryEntry::Write(FFile* writer)
{
    ...
    if (directories.size() > 0) {
        ...
        for (auto& dir : directories) {
            ...
            writer->write_mark<uint32_t>("DirectoryEntry_Names_" + ptr_to_string(&dir), 0);
            writer->write_mark<uint32_t>("Files_Names_" + ptr_to_string(&dir), 0);
            writer->write_mark<uint32_t>("DirectoryEntry_Table_" + ptr_to_string(&dir), 0);
            writer->write_mark<uint32_t>("Files_Table_" + ptr_to_string(&dir), 0);
        }
    }
